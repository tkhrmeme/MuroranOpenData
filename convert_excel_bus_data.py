...
出力データフォーマット：
    ...
    2.系統ー運行時刻データ
        ...
        Schedule : { 
                    SCHEDULE_DAYTYPE: Int; 運行曜日(0: 平日，1: 土日祝)
...
# TODO:運行曜日判定
WORKDAY_XF_VALUES = [75,79,80,86,87]
HOLIDAY_XF_VALUES = [78,81,82,83,88,89]
...
#             header.append(int(cell.value))
    ...
    for ncol in range(COL_DATA_START, sheet.ncols):
        ...
        if cells[ROW_HEADER].ctype == xlrd.XL_CELL_EMPTY:
            ...
            op = {}
            # TODO:運行曜日判定
            if cells[ROW_HEADER].xf_index in WORKDAY_XF_VALUES:
                op[SCHEDULE_DAYTYPE] = DAYTYPE_WORKDAY
            elif cells[ROW_HEADER].xf_index in HOLIDAY_XF_VALUES:
